      ...
      {cards.map((card, i) => (
        <div key={i} className={cn(card.className, "min-h-[300px]")}>
          <motion.div
            ...
            className={cn(
              card.className,
              "relative overflow-hidden rounded-xl border bg-card text-card-foreground h-full w-full min-h-[300px] p-6",
              selected?.id === card.id
          ...
          >
            <div className="relative z-20 w-full">{card.content}</div>
          </motion.div>
        </div>
      ))}
      <motion.div
